describe('Create category', () => {
    ...
    it("Create category with required data and include in menu", () => {
        cy.loadDatabaseDump('empty_categories_dump')
        cy.goToCategories(adminEmail, adminPassword)
        cy.get('.button').children().contains('New Category').click()
        cy.location('pathname').should('eq', '/admin/categories/new')
        cy.get('.form-field-container').filter(':contains("Name")').find('input').type('cos')
        cy.get('.form-field-container').filter(':contains("Url key")').find('input').type('cos')
        cy.get('.card-section:contains("Include In Store Menu")').find('label:contains("Yes")').click()
        cy.get('button').contains('Save').click()
        cy.location('pathname').should('include', '/admin/categories/edit')
        cy.get('.form-field-container').filter(':contains("Name")').find('input').should('have.value', 'cos')
        cy.get('.form-field-container').filter(':contains("Url key")').find('input').should('have.value', 'cos')
        cy.get('.card-section:contains("Include In Store Menu")').find('label:contains("Yes")').find('.radio-checked').should('exist')
        cy.get('.card-section:contains("Include In Store Menu")').find('label:contains("No")').find('.radio-checked').should('not.exist')
        cy.get('.breadcrum-icon').click()
        cy.location('pathname').should('eq', '/admin/categories')
        cy.get('.listing > tbody').find('tr').should('have.length', 2)
        cy.get('.listing > tbody').find('tr').eq(1).find('td').eq(1).should('have.text', 'cos')
        cy.get('.listing > tbody').find('tr').eq(1).find('td').eq(3).should('have.text', 'Yes')
    })

    it("Create category with required data and not include in menu", () => {
        cy.loadDatabaseDump('empty_categories_dump')
        cy.goToCategories(adminEmail, adminPassword)
        cy.get('.button').children().contains('New Category').click()
        cy.location('pathname').should('eq', '/admin/categories/new')
        cy.get('.form-field-container').filter(':contains("Name")').find('input').type('cos')
        cy.get('.form-field-container').filter(':contains("Url key")').find('input').type('cos')
        cy.get('.card-section:contains("Include In Store Menu")').find('label:contains("No")').click()
        cy.get('button').contains('Save').click()
        cy.location('pathname').should('include', '/admin/categories/edit')
        cy.get('.form-field-container').filter(':contains("Name")').find('input').should('have.value', 'cos')
        cy.get('.form-field-container').filter(':contains("Url key")').find('input').should('have.value', 'cos')
        cy.get('.card-section:contains("Include In Store Menu")').find('label:contains("Yes")').find('.radio-checked').should('not.exist')
        cy.get('.card-section:contains("Include In Store Menu")').find('label:contains("No")').find('.radio-checked').should('exist')
        cy.get('.breadcrum-icon').click()
        cy.location('pathname').should('eq', '/admin/categories')
        cy.get('.listing > tbody').find('tr').should('have.length', 2)
        cy.get('.listing > tbody').find('tr').eq(1).find('td').eq(1).should('have.text', 'cos')
